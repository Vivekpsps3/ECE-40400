...
sub encrypt() {
    ...
    my $round_key = extract_round_key( $key );
    my $bv = Algorithm::BitVector->new( filename => 'filename.txt' );
    while ($bv->{more_to_read}) {
        my $bitvec = $bv->read_bits_from_file( 64 );
        my ($LE, $RE) = $bitvec.divide_into_two();
        my $newRE = $RE.permute( \@expansion_permutation );
        my $out_xor = $newRE ^ $round_key;
        ...
        # either
        #
        #        my $ final_string = $RE followed by ($RE_modified xored with $LE)
        # or
        #        my $final_string = $LE followed by ($LE_modified xored with $RE)
        #
